...
public sealed class CreateClinicalTrialRequestHandler(IMapper mapper, IUnitOfWork repository)
    : IRequestHandler<CreateClinicalTrialRequest, ClinicalTrialDto>
{
    public async Task<ClinicalTrialDto> Handle(CreateClinicalTrialRequest request, CancellationToken cancellationToken)
    {
        var json = await ReadFileAsync(request.File);
        ...
        await repository.ClinicalTrialRepository.AddAsync(clinicalTrial);
